...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        : stop_words_(MakeUniqueNonEmptyStrings(stop_words)) {
            for (const string& word : stop_words_){
                if (!IsValidWord(word)){
                    throw invalid_argument("Недопустимые знаки при добавлении стоп-слов"s);
                }
            }
        }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        if (!IsValidWord(document)){
            throw invalid_argument("Попытка добавить документ c недопустимыми символами"s);
        }
    ...
    vector<Document> FindTopDocuments(const string& raw_query,DocumentPredicate document_predicate) const {
        ...
        sort(matched_documents.begin(), matched_documents.end(),[](const Document& lhs, const Document& rhs) {
            if (abs(lhs.relevance - rhs.relevance) < COMPARISON_ACCURACY) {
   ...
   tuple<vector<string>,DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
    ...
    int GetDocumentId(int index) const {
       if (index < 0 || index > documents_.size()){
           throw out_of_range("Нет документа с таким индексом"s);
       }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text.empty() || text[0] == '-'){
            throw invalid_argument("Невалидный поисковый запрос"s);
        }
    ...
    Query ParseQuery(const string& text) const {
        ...
        for (const string& word : SplitIntoWords(text)) {
            if (!IsValidWord(word)){
                throw invalid_argument("В поисковом запросе недопустимые символы"s);
                }
...
int main() {
    ...
    cout << "invalid_argument: "s << e.what() << endl; 
        ...
        cout << "out_of_ranget: "s << e.what() << endl; 
    }    
}